...
Each packet includes the acceptance condition, allowed scope, relevant Spec/Plan/ADR excerpts, expected verification, and the rule that the worker reports rather than marks completion. For ignored Spec artifacts, inject these excerpts explicitly; another worktree cannot be assumed to contain them.

For every result, verify changed paths, tests or other evidence, residual risk, and dependency effects. Mark `[X]` only after verification. Retry bounded failures; leave failed tasks incomplete and block their dependents.
