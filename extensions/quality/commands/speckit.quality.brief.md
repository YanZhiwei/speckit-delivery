...
RULES:
- <format/lint/type/test/complexity/comment rule>
ARCHITECTURE:
...
The brief is advisory context, not a pass. If no profile matches a changed
source path, say `PROFILE_GAP: <path>` and instruct the caller to stop or
